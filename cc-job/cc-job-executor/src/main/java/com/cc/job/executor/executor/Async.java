package com.cc.job.executor.executor;


import com.cc.job.executor.constants.JobConstant;
import com.cc.job.executor.executor.callback.ICallback;
import com.cc.job.executor.executor.callback.IWorker;
import com.cc.job.executor.executor.timer.SystemClock;
import com.cc.job.executor.executor.worker.DependWrapper;
import com.cc.job.executor.executor.worker.ResultState;
                        ...
                        if (nextWrappers != null && !nextWrappers.isEmpty()) {
                            ...
                            for (String nextId : nextIds) {
                                ...
                                inDegree.compute(nextId, (k, i) -> {
                                    ...
                                    if (i <= 0) {
                                        logger.debug("任务 {} 的入度已经是0或负数: {}，保持不变", nextId, i);
                                        return i;
                                    }
                                    int updated = i - 1;
                                    logger.info("任务 {} 的入度: {} -> {}", nextId, i, updated);
                                    if (updated == 0) {
                                        logger.info("✅✅✅ 任务 {} 的入度变为0，可以执行了 ✅✅✅", nextId);
