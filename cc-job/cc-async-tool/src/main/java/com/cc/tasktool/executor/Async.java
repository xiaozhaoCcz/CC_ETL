...
public class Async {
    ...
    public static boolean beginWork(long timeout, ExecutorService executorService, List<WorkerWrapper> workerWrappers)
        ...
        try {
            ...
            stopWork(workerWrappers.toArray(new WorkerWrapper[0]));
            return false;
        }
    }
     ...
     * 停止任务
     */
    public static void stopWork(WorkerWrapper... workerWrappers) {
        Set<WorkerWrapper> set = new HashSet<>();
        totalWorkers(Arrays.asList(workerWrappers), set);
