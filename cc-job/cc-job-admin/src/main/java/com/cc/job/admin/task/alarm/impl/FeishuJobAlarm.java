...
/**
 * 飞书机器人告警：任务失败时向飞书群发送文本消息（msg_type: text）
 */
@Component
public class FeishuJobAlarm implements JobAlarm {
    ...
    private final AlarmFeishuProperties properties;
    private final RestTemplate restTemplate = new RestTemplate();

    public FeishuJobAlarm(AlarmFeishuProperties properties) {
    ...
    public boolean doAlarm(JobInfo info, JobLog jobLog) {
        ...
        try {
            ...
            if (!response.getStatusCode().is2xxSuccessful()) {
                logger.warn("[FeishuAlarm] webhook returned {}", response.getStatusCode());
                return false;
            }
            return true;
        } catch (Exception e) {
            logger.error("[FeishuAlarm] send failed, jobLogId={}", jobLog.getId(), e);
            return false;
        }
    }
}
