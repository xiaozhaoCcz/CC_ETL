            ...
            public void run() {
                ...
                while (!scheduleThreadToStop) {
                    ...
                    try {
                        ...
                        List<JobInfo> scheduleList = XxlJobAdminConfig.getAdminConfig().getJobInfoMapper().scheduleJobQuery(nowTime + PRE_READ_MS, preReadCount);
                        if (scheduleList!=null && scheduleList.size()>0) {
                            ...
                            for (JobInfo jobInfo: scheduleList) {
                                ...
                                if (nowTime > jobInfo.getTriggerNextTime() + PRE_READ_MS) {
                                    ...
                                    if (MisfireStrategyEnum.FIRE_ONCE_NOW == misfireStrategyEnum) {
                                        ...
                                        JobTriggerPoolHelper.trigger(jobInfo.getId().intValue(), TriggerTypeEnum.MISFIRE, -1, null, null, null,0,null);
                                    ...
                                    // 1、trigger
                                    JobTriggerPoolHelper.trigger(jobInfo.getId().intValue(), TriggerTypeEnum.CRON, -1, null, null, null,0,null);
                                    logger.debug(">>>>>>>>>>> xxl-job, schedule push trigger : jobId = " + jobInfo.getId() );
                            ...
                            for (JobInfo jobInfo: scheduleList) {
                                XxlJobAdminConfig.getAdminConfig().getJobInfoMapper().updateById(jobInfo);
                            }
                    ...
                    try {
                        ...
                        if (ringItemData.size() > 0) {
                            ...
                            for (Long jobId: ringItemData) {
                                // do trigger
                                JobTriggerPoolHelper.trigger(jobId.intValue(), TriggerTypeEnum.CRON, -1, null, null, null,0,null);
                            }
                            // clear
                            ringItemData.clear();
                        }
